...
class StmMap<K, V> implements OurMap<K, V> {
    ...
    public V put(K k, V v) {
            ...
            atomic(() -> {
                ...
                ItemNode<K, V> item = bs[hash].get();
                ItemNode<K, V> newItem;

                newItem = item;
                // item = item.next;
                while (item != null) {
                    newItem = new ItemNode<K, V>(item.k, item.v, item.next);
                    newItem = newItem.next;
                }
                newItem = new ItemNode<K, V>(k, v, null);
            }});
        atomic(() -> {
            final TxnRef<ItemNode<K, V>>[] bs = buckets.get();
            final int h = getHash(k), hash = h % bs.length;
            bs[hash].set(new ItemNode<>(k, v, null));
        });
        return v;
    }
    ...
    public V putIfAbsent(K k, V v) {
        ...
        if (out[0] == 1) return v; return null;
    }
    ...
    public V remove(K k) {
            atomic(() -> {
                if (containsKey(k)) {
                    final TxnRef<ItemNode<K, V>>[] bs = buckets.get();
                    final int h = getHash(k), hash = h % bs.length;
                    Holder<V> newlist = new Holder<V>();
                    bs[hash].set(ItemNode.delete(bs[hash].get(), k, newlist));
                }});
        return null;
    }
    ...
    public void forEach(Consumer<K, V> consumer) {
        atomic(() -> {
            ...
            for (int hash = 0; hash < bs.length; hash++) {
                ...
                ItemNode<K, V> node = bs[finalHash].get();
                while (node != null) {
