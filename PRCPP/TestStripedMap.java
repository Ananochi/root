...
class TestStripedMap {
    ...
    private static void concurrentTests() {
        final CyclicBarrier barrier;
        final StripedWriteMap stripedWriteMap = new StripedWriteMap<Integer, String>(77,7);
        final WrapConcurrentHashMap wrapConcurrentHashMap = new WrapConcurrentHashMap();
        ...
        for (int t=0; t<=15; t++) {
            ...
            threads[t] = new Thread(() -> {
                ...
                Mark7(String.format("%-21s %d", "StripedWriteMap", threadCount),
                        i -> concurrentTest(threadCount,stripedWriteMap));
                Mark7(String.format("%-21s %d", "WrapConcHashMap", threadCount),
                        i -> concurrentTest(threadCount, wrapConcurrentHashMap));
        ...
        for (int t=0; t<threads.length; t++)
            threads[t].start();
        stripedWriteMap.reallocateBuckets();
        wrapConcurrentHashMap.reallocateBuckets();
        try {
            for (int t=0; t<threads.length; t++)
                threads[t].join();
    ...
    private static double concurrentTest(int threadCount, final OurMap<Integer, String> map) {
        final RandomKeysGenerator generator = new RandomKeysGenerator(new Random());
        //System.out.printf("%n Thread id: %s%n", threadCount);
        for (int i =0; i<10;i++){
            Pair<String,Integer> generatedPair = generator.GenerateEntry();
            map.put(generatedPair.getValue(),generatedPair.getKey());
        }
        concurrentTestMap(map);
        return map.size();
    }

    static class RandomKeysGenerator{
        private Random _random;
        RandomKeysGenerator(Random random){
            _random = random;
        }
        ...
        public Pair<String,Integer> GenerateEntry(){
            int FairRandomNumber = _random.nextInt(max - min + 1) + min;
            return new Pair<>(String.format("MapEntry%s",FairRandomNumber),FairRandomNumber);
        }
    }
...
interface OurMap<K,V> {
    ...
    OurMap<K,V> Create(int bucketCount,int lockCount );
}
...
class SynchronizedMap<K,V> implements OurMap<K,V>  {
    ...
    @Override
    public OurMap<K, V> Create(int bucketCount, int lockCount) {
        return new SynchronizedMap<>(bucketCount);
    }
...
class StripedMap<K,V> implements OurMap<K,V> {
    ...
    @Override
    public OurMap<K, V> Create(int bucketCount, int lockCount) {
        return new StripedMap<>(bucketCount,lockCount);
    }
...
class StripedWriteMap<K,V> implements OurMap<K,V> {
    ...
    @Override
    public OurMap<K, V> Create(int bucketCount, int lockCount) {
        return new StripedWriteMap<>(bucketCount,lockCount);
    }
    // First lock all stripes.  Then double bucket table size, rehash,
    // and redistribute entries.  Since the number of stripes does not
    // change, and since buckets.length is a multiple of lockCount, a
...
class WrapConcurrentHashMap<K,V> implements OurMap<K,V> {
    ...
    @Override
    public OurMap<K, V> Create(int bucketCount, int lockCount) {
        return new WrapConcurrentHashMap<>();
    }
}
